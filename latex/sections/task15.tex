...
The SF static model with $\gamma\!=\!2.5$ matches the predicted $\tau_S=\gamma/(\gamma-1)=5/3$ exponent ($\text{CCDF}\sim S^{-2/3}$, much steeper than MF), while $\gamma\!=\!3.0$ already approaches the MF regime~\cite{goh2003sandpile}.
While slightly different from what expected for heterogeneous topologies, we get a qualitatively consistent pattern of  for more heterogeneous networks, with a consistent pattern but not in the $S^{-2}$ regime.
